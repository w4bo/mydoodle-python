...
class Doodle:
    ...
    def __init__(self, url=None, poll_id=None):
        ...
        self.json_file = None
        self.update()
        tz = self.json_file['initiator'].get('timeZone')  # sometimes the api provides a tz,
                                                          # sometimes it doesn't,
                                                          # not sure why
        self.timezone = pytz.timezone(tz) if tz else timezone(timedelta(hours=-12))  # todo -12 is system specific
    ...
    def options(self) -> list:
        ...
        return [(t2d(p['date'] / 1000), t2d(p['start'] / 1000), p['text']) for p in self.json_file["options"]]
    ...
    def participants(self) -> list:
        return [(p['name'], p['preferences']) for p in self.json_file["participants"]]
